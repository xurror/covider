...
describe("Authentication reducer tests", () => {
  ...
  describe("Requests", () => {
    it("should detect a request", () => {
      expect(
        authentication(undefined, { type: REQUEST(ACTION_TYPES.LOGIN) })
      ).toMatchObject({
        loading: true,
      });
      expect(
        authentication(undefined, { type: REQUEST(ACTION_TYPES.GET_SESSION) })
      ).toMatchObject({
        loading: true,
      });
    });
  ...
  describe("Failure", () => {
    ...
    it("should detect a failure", () => {
      const payload = "Something happened.";
      const toTest = authentication(undefined, {
        type: FAILURE(ACTION_TYPES.GET_SESSION),
        payload,
      });
