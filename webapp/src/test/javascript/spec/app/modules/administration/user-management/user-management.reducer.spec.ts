...
describe("User management reducer tests", () => {
  ...
  describe("Requests", () => {
    it("should not modify the current state", () => {
      testInitialState(
        userManagement(undefined, { type: REQUEST(ACTION_TYPES.FETCH_ROLES) })
      );
    });

    it("should set state to loading", () => {
      testMultipleTypes(
        [REQUEST(ACTION_TYPES.FETCH_USERS), REQUEST(ACTION_TYPES.FETCH_USER)],
        {},
        (state) => {
          expect(state).toMatchObject({
            errorMessage: null,
    ...
    it("should set state to updating", () => {
      testMultipleTypes(
        [
          REQUEST(ACTION_TYPES.CREATE_USER),
          REQUEST(ACTION_TYPES.UPDATE_USER),
          REQUEST(ACTION_TYPES.DELETE_USER),
        ],
        {},
        (state) => {
          expect(state).toMatchObject({
  ...
  describe("Actions", () => {
    ...
    it("dispatches FETCH_USERS_PENDING and FETCH_USERS_FULFILLED actions with pagination options", async () => {
      ...
      await store
        .dispatch(getUsers(1, 20, "id,desc"))
        .then(() => expect(store.getActions()).toEqual(expectedActions));
    });
    ...
    it("dispatches UPDATE_USER_PENDING and UPDATE_USER_FULFILLED actions", async () => {
      ...
      await store
        .dispatch(updateUser({ login: username }))
        .then(() => expect(store.getActions()).toEqual(expectedActions));
    });
