...
describe("Activate reducer tests", () => {
  ...
  it("should reset", () => {
    expect(
      activate(
        { activationSuccess: true, activationFailure: false },
  ...
  it("should detect a success", () => {
    expect(
      activate(undefined, { type: SUCCESS(ACTION_TYPES.ACTIVATE_ACCOUNT) })
    ).toMatchObject({
  ...
  it("should return the same state on request", () => {
    expect(
      activate(undefined, { type: REQUEST(ACTION_TYPES.ACTIVATE_ACCOUNT) })
    ).toMatchObject({
      activationSuccess: false,
      activationFailure: false,
    });
  ...
  it("should detect a failure", () => {
    expect(
      activate(undefined, { type: FAILURE(ACTION_TYPES.ACTIVATE_ACCOUNT) })
    ).toMatchObject({
      activationSuccess: false,
