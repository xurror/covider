...
import cm.ubuea.covider.config.audit.AuditEventConverter;
...
public class CustomAuditEventRepository implements AuditEventRepository {
    ...
    public List<AuditEvent> find(final String principal, final Instant after, final String type) {
        final Iterable<PersistentAuditEvent> persistentAuditEvents = persistenceAuditEventRepository
    ...
    public void add(final AuditEvent event) {
        if (!AUTHORIZATION_FAILURE.equals(event.getType()) && !Constants.ANONYMOUS_USER.equals(event.getPrincipal())) {
            ...
            final Map<String, String> eventData = auditEventConverter.convertDataToStrings(event.getData());
